...
class UsernameEditViewModel : BaseViewModel {
	...
	override init() {
		...
		username.asObservable().distinctUntilChanged().subscribe({ [weak self] (username) in
	...
	var buttonObservable: Observable<Bool> {
		return Observable.combineLatest(isTaken.asObservable(),
																		isLoading.asObservable(),
																		isTakenLoading.asObservable()).map({ (val) -> Bool in
			return !val.0
				&& !val.1
				&& !val.2
				&& (self.username.value ?? "") != (Session.shared.user.value?.username ?? "")
		})
	}
	...
	func createSection() {
		...
		let username = TextFieldTableViewCellItem(reuseIdentifier: "TextFieldTableViewCell",
																							identifier: "TextFieldTableViewCell_Username")
	...
	func update(username: String) {
		...
		guard LoginForm.isUsernameValid(username: username)
